...
from core.models import predrnn, ConvLSTM
...
class Model(object):
    ...
    def train(self, data, mask, itr):
        frames = data  # (4,20,1,64,64)
        self.network.train()
        frames_tensor = torch.FloatTensor(frames).to(self.configs.device)  # (4,20,1,64,64)
        mask_tensor = torch.FloatTensor(mask).to(self.configs.device)  # (4,9,64,64,1)
